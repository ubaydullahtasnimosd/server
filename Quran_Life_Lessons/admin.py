...
class QuranLifeLessonsAdminForm(forms.ModelForm):
    upload_lesson_image = forms.ImageField(
        required=False,
        label="নতুন ছবি আপলোড করুন (Upload Image)",
        help_text="মোবাইল বা কম্পিউটার থেকে ছবি সিলেক্ট করুন। অনুমোদিত ফরম্যাট: JPG, PNG, JPEG, WEBP (সর্বোচ্চ ২ MB)। এটি Supabase এর 'Life lessons from the Quran' বাকেটে জমা হবে।",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "quranLessonImg" in self.fields:
            self.fields["quranLessonImg"].required = False
            self.fields["quranLessonImg"].label = "ছবির লিংক / URL (স্বয়ংক্রিয়ভাবে তৈরি হবে অথবা ম্যানুয়াল লিংক)"


@admin.register(Quran_Life_Lessons)
class QuranLifeLessonsAdmin(ModelAdmin):
    form = QuranLifeLessonsAdminForm
    ...
    fieldsets = (
        ("কুরআন পাঠের তথ্য (Quran Lesson Information)", {
            "fields": (
                "upload_lesson_image",
                "current_lesson_image",
                "quranLessonImg",
    ...
    def current_lesson_image(self, obj):
        image_url = get_image_url(obj.quranLessonImg) if obj else None
        if not image_url:
    ...
    def save_model(self, request, obj, form, change):
        ...
        if image_file:
            uploaded_url = upload_file_to_supabase(
                image_file,
                bucket_name="Life lessons from the Quran",
                folder="quran_lessons"
            )
            if uploaded_url:
                obj.quranLessonImg = uploaded_url
