...
class HadithLifeLessonsAdminForm(forms.ModelForm):
    upload_lesson_image = forms.ImageField(
        required=False,
        label="নতুন ছবি আপলোড করুন (Upload Image)",
        help_text="মোবাইল বা কম্পিউটার থেকে ছবি সিলেক্ট করুন। অনুমোদিত ফরম্যাট: JPG, PNG, JPEG, WEBP (সর্বোচ্চ ২ MB)। এটি Supabase এর 'Life lessons from the Hadith' বাকেটে জমা হবে।",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "hadithLessonImg" in self.fields:
            self.fields["hadithLessonImg"].required = False
            self.fields["hadithLessonImg"].label = "ছবির লিংক / URL (স্বয়ংক্রিয়ভাবে তৈরি হবে অথবা ম্যানুয়াল লিংক)"


@admin.register(Hadith_Life_Lessons)
class HadithLifeLessonsAdmin(ModelAdmin):
    form = HadithLifeLessonsAdminForm
    ...
    fieldsets = (
        ("হাদিস পাঠের তথ্য (Hadith Lesson Information)", {
            "fields": (
                "upload_lesson_image",
                "current_lesson_image",
                "hadithLessonImg",
    ...
    def current_lesson_image(self, obj):
        image_url = get_image_url(obj.hadithLessonImg) if obj else None
        if not image_url:
    ...
    def lesson_image_preview(self, obj):
        image_url = get_image_url(obj.hadithLessonImg)
        if not image_url:
            return "-"
        return format_html(
    ...
    def save_model(self, request, obj, form, change):
        ...
        if image_file:
            uploaded_url = upload_file_to_supabase(
                image_file,
                bucket_name="Life lessons from the Hadith",
                folder="hadith_lessons"
            )
            if uploaded_url:
                obj.hadithLessonImg = uploaded_url
